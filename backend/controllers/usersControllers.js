  ...
  if (req.query.tokenAuth) {
    UsersModel.findOne({ tokenAuth: req.query.tokenAuth }, function (err, doc) {
      if (err)
        return res
          .status(200)
          .send({ status: 400, message: "ERROR get data DB !" });
      console.log(doc);
      if (!doc || err)
        return res
          .status(200)
    ...
    UsersModel.find((err, docs) => {
      if (!err) {
        ...
        res.status(200).send({ status: 400, message: "ERROR get data DB !" });
    });
  }
});
  ...
  UsersModel.findOne({
    ...
    .then(function (doc) {
      if (doc && doc.mail == req.body.mail)
        return res
          .status(200)
          .send({ status: 401, message: "Mail already use !" });
      if (doc && doc.username == req.body.username)
        return res
          .status(200)
          .send({ tatus: 402, message: "Username already use !" });
      var reg = new RegExp(
        "^[a-z0-9]+([_|.|-]{1}[a-z0-9]+)*@[a-z0-9]+([_|.|-]{1}[a-z0-9]+)*[.]{1}[a-z]{2,6}$",
        "i"
      );
      if (!reg.test(req.body.mail))
        return sendMessageError(res, 404, "Unvalid Mail !");
      ...
      newUser
        ...
        .then(function (docs) {
          ...
          UsersModel.updateOne({ _id: docs._id }, { tokenAuth: token }).then(
            function (suc) {
              return res
                .status(200)
  ...
  UsersModel.findOne(
    ...
    function (err, doc) {
      ...
      if (doc.password != req.body.password)
        return res
          .status(202)
          .send({ status: 202, message: "Password no valid !" });
      if (doc.password == req.body.password)
...
router.post("/login/google", (req, res) => {
  UsersModel.findOne(
    { $and: [{ mail: req.body.mail }, { google: true }] },
    function (err, doc) {
      if (err)
        return res
          .status(200)
...
router.get("/widget", (req, res) => {
  UsersModel.findOne({ tokenAuth: req.query.tokenAuth })
    ...
    .catch(function (err) {
      return res
        .status(200)
        .send({ status: 400, message: "Not possible get widget !" });
    });
});
...
router.delete("/widget", (req, res) => {
  ...
  UsersModel.findOne({ tokenAuth: req.body.tokenAuth })
    ...
    .catch(function (err) {
      return res
        .status(200)
        .send({ status: 400, message: "The User Token doesn't exist !" });
    });
});
